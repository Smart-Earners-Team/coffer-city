...
const Dashboard = () => {
  ...
  const getActivityStatus = async () => {
    ...
    const promises = balanceArr.map(async ([address, value]) => {
      ...
      const amt = new BigNumber(Number(value))
        .div(new BigNumber(10).pow(Number(assetDecimals)))
        .toNumber();
      // console.log(amt);

      // const priceUsd: number = await fetchTokenPairs(address, String(chain?.network));

      const priceUsd: number = await fetchTokenPairs(
        String(address),
            ...
            <div className="md:flex w-full gap-5 relative justify-center">
              <div className="p-4 grid gap-1 relative w-full md:w-[30%] rounded-xl bg-[#02075d] text-slate-50">
                ...
                <button
                  ...
                  className="absolute text-lg right-5 h-full py-5 top-0"
                >
              ...
              <div className="grid gap-3 text-xs py-3 md:py-0">
                <Link
                  to={"/overview"}
                  className="font-bold p-2 rounded-lg bg-[#02075d] text-slate-50 h-fit m-auto hover:bg-[#02075d]/90 duration-300"
                >
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-5">
              <Link
                ...
                className={`rounded-lg ${
                  isActive ? "bg-[#27A844]/10" : "bg-[#9d2b3c]/10"
                } p-5 grid gap-5 items-center`}
              >
                <span
                  className={`rounded-full w-fit h-fit bg-slate-50 ${
                    isActive ? "text-[#27A844]" : "text-[#9d2b3c]"
                  }`}
                >
                  {isActive ? (
                    <BsShieldFillCheck className="m-3 text-xl" />
                  ) : (
                    <BsShieldFillExclamation className="m-3 text-xl" />
                  )}
                </span>
              ...
              <Link
                to="/rewards"
                className="rounded-lg bg-[#02075d]/10 p-5 grid gap-5 items-center"
              >
              ...
              <Link
                to="/overview"
                className="rounded-lg bg-[#FFD700]/10 p-5 grid gap-5 items-center"
              >
              ...
              <Link
                to="/deposit"
                className="rounded-lg bg-[#795548]/10 p-5 grid gap-5 items-center"
              >
                ...
                <div className="flex flex-col justify-center">
                  ...
                  <div className="text-xs">
                    Start saving for that well-deserved vacation or your dream
                    home.
                  </div>
                </div>
              </Link>
